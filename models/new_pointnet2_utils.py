...
def sample_and_group(npoint, radius, nsample, xyz, points, returnfps=False):
    ...
    B, N, C = xyz.shape
    S = npoint
    fps_idx = farthest_point_sample(xyz, npoint) # [B, npoint, C]   seems to be [B, npoints]
    new_xyz = index_points(xyz, fps_idx)     # [B, npoint, C]
    idx = query_ball_point(radius, nsample, xyz, new_xyz)   # [B, npoint, nsample]  for each centroid, there are nsample neighbors
    ...
    def forward(self, xyz, points):
        ...
        for i, radius in enumerate(self.radius_list):
            ...
            for j in range(len(self.conv_blocks[i])):
                ...
                conv_weight_ori = conv.weight
                bn = self.bn_blocks[i][j]
                grouped_points =  F.relu(bn(conv(grouped_points)))
                conv.weight = conv_weight_ori
